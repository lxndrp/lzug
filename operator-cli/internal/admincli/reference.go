...
func GenerateReference(registry *Registry) string {
	...
	output.WriteString("Die entfernte Option `--container`, die Umgebungsvariable `LZUG_ADMIN_CONTAINER` und das gleichnamige JSON-Feld werden mit einer Migrationsmeldung abgewiesen; sie wählen niemals stillschweigend den Standard-Endpunkt.\n")
	...
	for _, command := range registry.Commands() {
		...
		if command.Transport == LocalTransport {
			if command.UsesConfig {
				output.WriteString("Ausführung: lokale Orchestrierung mit Backendaufträgen; geheimes Schlüsselmaterial verbleibt in der CLI.\n\n")
			} else {
				output.WriteString("Ausführung: lokal ohne Backendauftrag.\n\n")
			}
		} else {
