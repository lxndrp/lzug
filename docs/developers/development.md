...
| Go-CLI | `task test:operator` oder `task quality:operator`; für native Archive `task quality:operator-packaging`, für bytegleiche Reproduzierbarkeit `task quality:operator-reproducibility` |
