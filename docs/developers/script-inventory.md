...
| `docs/publication.py` | Dokumentation/Publikation; `task docs:publication*`, `task docs:wiki:post-publish` und der Publication-Workflow | Montiert den aktuellen Pages-Stand sowie die unaktivierten Wiki-/Pages-Kandidaten, prüft Wiki-spezifische Routen und bindet konkrete Rückfallstände. Inventar und Kandidaten entstehen nur unter `build/`; generische Markdown-, Link- und Buildaufgaben bleiben bei Gollum, Hugo, TypeDoc und Lychee. |
| `docs/publication/check-wiki-render.rb` | Dokumentation/Publikation; `task docs:publication:candidate:check` | Erzwingt im gelockten Gollum-Container das tatsächliche Parsen und Rendern jeder flachen Kandidatenseite. Behalten als kleiner GitHub-Wiki-spezifischer Renderingadapter; er enthält weder Inhalte noch eine Publikationsfunktion. |
