...
| `docs/publication.py` | Dokumentation/Publikation; `task docs:publication*` und der Publication-Workflow | Montiert den bestehenden Pages-Stand mit Hugo, OpenAPI, TypeDoc und den generierten Referenzen. Das Profil `current|candidate` reduziert ausschließlich den Pages-Kandidaten; Wiki-Kopie und generische Linkprüfung bleiben sichtbar im Taskfile bei Standardwerkzeugen. Die vollständige Ablösung dieses älteren Assemblers ist in #770 beauftragt. |
