...
- Shell-Smokes orchestrieren Docker und belegen am realen Image
...
InSpec 7 liefert Docker-Ressourcen als separates Ruby-Gem aus.
Damit müsste `lzug` zusätzlich ein Runtime-spezifisches Resource-Pack sowie die Ruby-/InSpec-Laufzeit pflegen.
...
Das offizielle Spike-Image ist ausschließlich `linux/amd64`, während das lzug-Produktimage auch `linux/arm64` unterstützt.
...
| --- | --- | --- |
| Image und Runtime | Docker-Smokes prüfen reale Runtime, Health und Metadaten | deklarativer, aber doppelter `inspect`-Nachweis |
...
vorhandene Assertions und den bestehenden Runtime-Vertrag duplizieren.
